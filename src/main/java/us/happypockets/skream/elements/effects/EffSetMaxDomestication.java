...
@Name("Set Max Domestication")
@Description({"Sets the max domestication level of a horse, skeleton horse, or zombie horse."})
@Examples("set the max domestication level of event-entity to 10")

public class EffSetMaxDomestication extends Effect {

    static {
        Skript.registerEffect(EffSetDomestication.class, "set [the] max (temper|domestication) [level] of [horse] %livingentity% to %integer%");
    }
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "Set max domestication level effect with expression livingentity: " + horse.toString(event, debug) +  " and expression integer " + integer.toString(event, debug);
    }
    ...
    protected void execute(Event event) {
        ...
        for (LivingEntity h : horse.getAll(event)) {
            if(h.getType().equals(EntityType.HORSE) || h.getType().equals(EntityType.SKELETON_HORSE) || h.getType().equals(EntityType.ZOMBIE_HORSE)){
                ...
                theHorse.setMaxDomestication(i);
            }
            else{
